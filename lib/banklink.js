...
module.exports.samplePayment = function(req, projectId, userId, urlPrefix, isAuth, options, callback) {
    db.findOne(
        'project',
        {
        ...
        (err, project) => {
            ...
            if (userId && !tools.checkAuthorized(req, project)) {
            ...
            bankObjects[banks[project.bank].type][method](req, project, urlPrefix, options || {}, callback);
        }
    );
};
    ...
    project.validateClient((err, data) => {
        ...
        if (err) {
            ...
            return logPayment(
                ...
                () => serveErrors(req, res, errors, warnings)
            );
        }
        if (!data.success) {
            return logPayment(project, 'ERROR', req, res, data, () => serveErrors(req, res, errors, warnings));
        }
            ...
            db.findOne(
                ...
                (err, project) => {
                    ...
                    return res.render('banklink/preview', {
                        ...
                        project,
                        isAuthorized: tools.checkAuthorized(req, project),
...
function autoPay(req, res, id, payment) {
    let ap = (req.body.PANGALINK_AUTOPAY || '')
            .toString()
            .trim()
            .toLowerCase(),
        options = {
                    ...
                    handlePayment(record, project, (err, response) => {
                        if (err) {
                            if (!project) {
                                ...
                                err.redirectUrl = '/banklink/preview/' + record._id;
                                return callback(err);
                            }
                        }
