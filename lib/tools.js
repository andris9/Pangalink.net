...
module.exports.renderDocs = function(name, callback) {
    ...
    fs.readFile(__dirname + '/../docs/' + name + '.md', 'utf-8', (err, content) => {
        ...
        try {
            rendered.set(name, marked(content));
        } catch (E) {
            return callback(E);
        }
...
module.exports.processAuthorized = (users, authorized) => {
    authorized = [].concat(authorized || []).map(str => (str || '').toString().trim()).filter(str => str);
...
module.exports.checkAuthorized = (req, project) =>
    project.owner === req.user.username || req.user.role === 'admin' || project.authorized.map(id => id.toString()).includes(req.user._id.toString());
