...
module.exports.processAuthorized = authorized => {
    let lines;
...
module.exports.checkAuthorized = (req, project) => {
    return project.owner === req.user.username || req.user.role === 'admin' || project.authorized.includes(req.user.username.toLowerCase().trim());
};
