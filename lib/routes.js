...
module.exports = function(app) {
    ...
    app.get('/projects/:pageNumber', requireLogin, serveProjects);
    app.get('/projects', requireLogin, serveProjects);

    app.get('/add-project', requireLogin, serveAddProject);
    app.post('/add-project', requireLogin, handleAddProject);

    app.get('/edit-project/:project', requireLogin, serveEditProject);
    app.post('/edit-project', requireLogin, handleEditProject);

    app.get('/delete-project/:project', requireLogin, serveDeleteProject);

    app.get('/project/:project/example/render/:type.php', requireLogin, serveRenderedExamplePayment);
    app.get('/project/:project/example/:type.php', requireLogin, serveExamplePayment);
    app.get('/project/:project/regenerate', requireLogin, handleRegenerateProjectCertificate);
    app.get('/project/:project/:key([^.]+).pem', requireLogin, serveKey);
    app.get('/project/:project', requireLogin, serveProject);
    app.post('/project/:project', requireLogin, serveProject);
    app.get('/project/:project/page/:pageNumber', requireLogin, serveProject);
    app.get('/project/:project/page', requireLogin, serveProject);
    app.get('/project/:project/:tab', requireLogin, serveProject);

    app.get('/payment/:payment/scripts/:direction([^.]+).php', requireLogin, servePayment);
    app.get('/payment/:payment', requireLogin, servePayment);
                    ...
                    if (['pay', 'receive'].indexOf(req.params.direction) >= 0) {
                        ...
                        serve(req, res, {
                            ...
                            values: {
                                ...
                                host: urllib.parse(
                                    payment.state === 'PAYED'
                                        ? payment.successTarget
                                        : payment.state === 'REJECTED' ? payment.rejectTarget : payment.cancelTarget
                                ).host,
