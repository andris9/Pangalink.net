        ...
        if (this.bank.key !== record.bank) {
            return callback(null, {
                ...
                errors: [{
                    field: 'VK_SND_ID',
                    value: (this.fields.VK_SND_ID || '').toString(),
                    error: util.format('Valitud kliendi tunnus kehtib ainult "%s" makselahenduse jaoks, hetkel on valitud "%s"', banks[record.bank].name, this.bank.name)
                }],
                warnings: false
            });
        }
...
IPizzaValidator.prototype.validateFields = function() {
    ...
    IPizza.serviceFields[service].forEach(field => {
        ...
        if (typeof response === 'string') {
            ...
            this.bank.fieldRegex[field] && !value.match(new RegExp(this.bank.fieldRegex[field].re, this.bank.fieldRegex[field].flags))) {
            this.errors.push({
                field: field,
                value: value,
...
IPizzaValidator.prototype.validate_VK_SERVICE = function() {
    ...
    if (!IPizza.serviceFields[value] || (this.bank.allowedServices && this.bank.allowedServices.indexOf(value) < 0)) {
        if (['1011', '1012', '4011', '4012'].indexOf(value) >= 0) {
            return util.format('Teenuskoodi %s ("%s") väärtus ei ole selle makselahenduse korral toetatud. Pangalingi uue protokolli kasutamiseks tuleb luua uus makselahendus.', 'VK_SERVICE', value);
        } else {
...
IPizzaValidator.prototype.validate_VK_DATETIME = function() {
    ...
    if (new Date(value) < new Date(Date.now() - 5 * 60 * 1000) || new Date(value) > new Date(Date.now() + 5 * 60 * 1000)) {
        this.warnings.push({
            field: 'VK_DATETIME',
            value: value,
            warning: util.format('Ajatempel %s ("%s") peab jääma vahemikku +/- 5 minutit serveri ajast ("%s")', 'VK_DATETIME', value, moment().format('YYYY-MM-DDTHH:mm:ssZZ'))
        });
    }
                ...
                fetchUrl(url, {
                    ...
                    if (err) {
                        payment.autoResponse = {
                            ...
                            method: method,
                            url: url,
                            fields: payload ? Object.keys(fields).map(key => {
                                return {
                                    key: key,
                                    value: fields[key]
                                };
