...
Solo.samplePayment = function(req, project, urlPrefix, options, callback) {
...
Solo.prototype.validateSignature = function(callback) {
    ...
    try {
        mac = crypto
            .createHash(this.record.soloAlgo)
            .update(this.sourceHash)
            .digest('hex')
            .toUpperCase();
    } catch (E) {
        return callback(E);
...
Solo.prototype.sign = function(callback) {
    ...
    try {
        this.fields[this.keyPrefix + 'RETURN_MAC'] = crypto
            .createHash(this.record.soloAlgo)
            .update(this.sourceHash)
            .digest('hex')
            .toUpperCase();
...
Solo.prototype.signClient = function(callback) {
    ...
    try {
        this.fields[this.keyPrefix + 'MAC'] = crypto
            .createHash(this.record.soloAlgo)
            .update(this.sourceHash)
            .digest('hex')
            .toUpperCase();
...
Solo.generateForm = function(payment, project, callback) {
    tools.incrTransactionCounter(project.uid, (err, transactionId) => {
        ...
        transaction.sign(err => {
            ...
            let payload = tools.stringifyQuery(fields, payment.charset);
            let url = payment.state === 'PAYED' ? payment.successTarget : payment.state === 'REJECTED' ? payment.rejectTarget : payment.cancelTarget;

            url += (url.match(/\?/) ? '&' : '?') + payload;

            if (payment.state === 'PAYED' && !!project.soloAutoResponse) {
                fetchUrl(
                    url,
                    {
                        method,
