...
function sendInvitation(req, invitation) {
    let html = ejs.render(template, {
        ...
        action: {
            link: `${req.siteProto}://${req.siteHostname}/account/join?username=${encodeURIComponent(invitation.address)}&ticket=${invitation.ticket}`,
    ...
    transport.sendMail(mailOptions, err => {
        if (err) {
            log.error('SMTP', 'Failed sending account ticket mail to %s, %s', invitation.address, err.message);
        } else {
            log.info('SMTP', 'Successfully sent ccount ticket mail to %s', invitation.address);
        }
    });
}
